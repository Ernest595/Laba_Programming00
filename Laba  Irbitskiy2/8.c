...
int sum_numbers8(int base, int count, ...);
int sum_base8(int base, char* num1, char* num2, char* result);

int main() {
	char* result;
	int err;
	err = sum_numbers8(2, 3, &result, "101", "1010", "100");
...
int sum_numbers8(int base, int count,  ...) {
	...
	for (i = 0; i < count; i++) {
		if ((ptr = va_arg(args, char*)) == NULL) {
			...
			data = sum_base8(base, a, ptr, result);
			switch (data)
			{
				case INCORRECT_ARG:
					return INCORRECT_ARG;
